...
RiggedObject::RiggedObject() : static_char(false),
                               primary_weapon_id(-1),
                               total_rotation(0.0f),
...
const mat4 RiggedObject::InterpolateBetweenTwoBones(const NetworkBone& current, const NetworkBone& next, float interp_step) {
    ...
    float scale = (1.0f - interp_step) * current.scale + interp_step * next.scale;

    float model_char_scale = (1.0f - interp_step) * current.model_char_scale + interp_step * next.model_char_scale;
...
void RiggedObject::HandleAnimationEvents(AnimationClient& _anim_client) {
    ...
    while (!active_events.empty()) {
        ...
        vec3 position = skeleton_.physics_bones[the_event.which_bone].bullet_object->GetPosition();
        ASArglist args;
...
enum LogReadStage { _deleted_faces,
                    _first_vert,
                    _second_vert,
                    _offset_x,
                    _offset_y,
